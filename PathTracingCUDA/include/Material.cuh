...
__device__ inline BSDFSample SampleGGXMicrofacetBRDF(const DielectricParams& params, const glm::vec3& wo, const HitRecord& si, curandState& RNG)
{
	...
	if (params.roughness < 0.01f) //delta case
	{
		glm::vec3 L = glm::reflect(-V, N);
		sample.wi = L;
...
__device__ inline BSDFSample SampleGGXMicrofacetBTDF(const DielectricParams& params, const glm::vec3& wo, const HitRecord& si, curandState& RNG)
{
	...
	halfway = (glm::dot(halfway, N) < 0.0f) ? -halfway : halfway;

	glm::vec3 L = glm::refract(-V, halfway, eta);
	if (glm::length(L) == 0.0f) return sample; //TIR
	...
	if (fabsf(glm::dot(N, V)) < 1e-6f || fabsf(glm::dot(N, L)) < 1e-6f) return sample;
...
__device__ inline BSDFSample SampleDielectricBSDF(const DielectricParams& params, const glm::vec3& wo, const HitRecord& si, curandState& RNG)
{
	...
	if (u < R) 
	{
		...
		if (params.roughness > 0.01f) s.pdf *= R;
		return s;
	}
	else 
	{
		...
		if (!s.good) atomicAdd(&rejected, 1ULL);
		if (params.roughness > 0.01f) s.pdf *= 1.0f - R;
		return s;
	}
}
